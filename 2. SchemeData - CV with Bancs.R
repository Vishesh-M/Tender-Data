...
endDate <- ymd('2019-10-16')
endDate <- ymd('2019-09-30')
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 13 - Merge BaNCS Payments ####
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# *13.1 - Inital loads ####
bancs <- read_excel('inputs/external payments datasets/BaNCS/ClaimPayment.xlsx', sheet=1)

# clean BaNCS
bancs_clean <- bancs %>% 
  mutate_at(vars('Payment from date', 'Payment to date', 'Claim Payment date'), dmy) %>% 
  filter(`Claim Payment date` <= dmy(30092019),
         `Claim payment status` == 'PAID',
         # exclude adhoc payments (already in CV Clean)
         !is.na(`Claim Number`)) %>%
  mutate(gross = replace_na(`Net claim amount paid`,0) + replace_na(`Payg Tax amount`,0)) %>% 
  group_by(`Claim Number`) %>% 
  # ~~~~~~~~ CHECK WHAT TO DO WITH FIRST/END/PROC DATES FOR $0 PAYMENTS ~~~~~
  summarise(BaNCS_Payment = sum(gross, na.rm = TRUE),
            # kept it as CV to avoid changing Bind script for now ~~~~ ALIGN NAMES LATER
            FirstEffPaymentCV = min(`Payment from date`, na.rm = T),
            LastEffPaymentCV = max(`Payment to date`, na.rm = T),
            LastProcessedDate = max(`Claim Payment date`))

# Cases where there are multiple BC's which could be associated with the payments
pas_dupl <- cv %>% 
  filter(!is.na(`PAS Claim`)) %>% 
  group_by(`PAS Claim`) %>% 
  filter(n() > 1) %>% 
  pull(`PAS Claim`)

cv <- cv %>% 
  # *13.2 - Merge 1-1 relationshio BaNCS x CV ####
  left_join(bancs_clean %>% 
              filter(!`Claim Number` %in% pas_dupl) %>% 
              rename(BaNCSPayment_11match = BaNCS_Payment),
            by=c('PAS Claim' = 'Claim Number')) %>% 
  # *13.3 - Merge M-1 relationshio BaNCS x CV ####
  # * ~~~~~ LEFT JOIN PAYMENTS MANUALLY LINKED BY ALI (template below) ~~~~~~~
  # left_join(bancs_clean %>% 
  #             filter(`Claim Number` %in% pas_dupl) %>% 
  #             rename(BaNCSPayment_M1match = BaNCSPayment),
  #           by=c('BCID' = 'RIGHT BC')) %>% 
  # keep it as AmountPaidTotal for now to avoid changing it in Bind script
  mutate(
    # BaNCS payments for 1-1 relationships
    AmountPaidTotal = replace_na(BaNCSPayment_11match,0) +
      # BaNCS payments for M-1 relationships (manually linked by ALI)
      # replace_na(BaNCSPayment_M1match,0) +
      
      # add Adhoc / Finance payments
      replace_na(jul_adhoc_gross, 0) + replace_na(jul_fin_gross, 0) +
      replace_na(sep_adhoc_gross, 0) + replace_na(sep_fin_gross, 0)
    )

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 14 - Merge Membership Data #### (VERY SLOW SECTION TO RUN)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# *14.1 - Load data ####
memb <- read_excel('inputs/external payments datasets/BaNCS/MemberData.xlsx', sheet=1)

# *14.2 - Clean membership data ####
# select only relevant columns
memb <- memb %>% 
  select(`Policy Number`, DateJoinedFund_Memb = `Date Joined Fund`, OccupationClass = `Member Occupation Class code`,
         DateOfBirth_Memb = `Date of Birth`, Gender_Memb = Gender) %>% 
  mutate_at(vars('DateJoinedFund_Memb', 'DateOfBirth_Memb'), dmy)

# removes duplicates (keeping latest record)
memb <- memb %>% 
  group_by(`Policy Number`) %>% 
  slice(which.max(row_number())) %>% 
  ungroup()

# *14.3 - Merge with CV Data ####
cv <- cv %>% 
  left_join(memb, by=c('Policy' = 'Policy Number')) %>% 
  mutate(DateJoinedFund = coalesce(DateJoinedFund_Memb, DateJoinedFund),
         BirthDate = coalesce(DateOfBirth_Memb, BirthDate),
         Gender = coalesce(recode(Gender_Memb, 
                                  'M' = 'Male',
                                  'F' = 'Female',
                                  'U' = NA_character_),
                           Gender))
